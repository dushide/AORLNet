...
import numpy as np
import torch
from util.loadMatData import load_data, features_to_Lap
...
def main(data, args):
    ...
    feature, labels, adj = load_data(args, data)
    n = feature.shape[0]
    n_feats = feature.shape[1]
    n_clusters = len(np.unique(labels))

    lap_Z = features_to_Lap(feature).to_dense().to(device)
    adj = torch.from_numpy(adj / 1.0).float().to(device)
    feature = torch.from_numpy(feature / 1.0).float().to(device)


    config = load_config(r'./config/DRLNet.yaml')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default=current_dir + "./result/DRLNet.txt",
    ...
    parser.add_argument('--block', type=int, default=2, help='The block number')
